...
// Module system
const modules = {
  logger: require('./logger.js') || { log: console.log },
...
async function startReporting(gameId) {
  ...
  while(true) {
    try {
      ...
      const response = await axios.get(config.urls.reportSystem, {
        httpsAgent: agent,
        headers: headers,
...
server.listen(8080, () => {
  clearConsole();
  console.log('\x1b[32m%s\x1b[0m', 'Server running on http://localhost:8080');
